...
aux.print(L['[TOOLTIPS_ADVANCED] Módulo de tooltips avanzados cargado'])
...
function M.mostrar_tooltip_avanzado(item_key, item_name, auction_info)
    ...
    -- Título
    GameTooltip:AddLine(item_name or L['Item desconocido'], 1, 0.82, 0)
    GameTooltip:AddLine(' ')
    
    -- Precio de mercado
    local market_value = history.value(item_key)
    if market_value and market_value > 0 then
        GameTooltip:AddDoubleLine(L['Precio de Mercado:'], format_gold(market_value), 1, 1, 1, 0.5, 1, 0.5)
    end
    
    -- Precio de la subasta (si está disponible)
    if auction_info and auction_info.buyout_price then
        GameTooltip:AddDoubleLine(L['Precio Subasta:'], format_gold(auction_info.buyout_price), 1, 1, 1, 1, 1, 1)
        ...
        if market_value and market_value > 0 then
            ...
            if discount > 0 then
                GameTooltip:AddDoubleLine(L['Descuento:'], string.format('%.0f%%', discount), 1, 1, 1, 0, 1, 0)
                GameTooltip:AddDoubleLine(L['Ganancia Potencial:'], format_gold(profit), 1, 1, 1, 0, 1, 0)
            else
                GameTooltip:AddDoubleLine(L['Sobreprecio:'], string.format('%.0f%%', -discount), 1, 1, 1, 1, 0.27, 0.27)
            end
        end
    end
    ...
    if market_analysis then
        ...
        if market_analysis.calculate_price_trend then
            ...
            if trend and direction then
                ...
                local trend_text = direction == 'rising' and L['↑ Subiendo'] or (direction == 'falling' and L['↓ Bajando'] or L['→ Estable'])
                GameTooltip:AddDoubleLine(L['Tendencia (7d):'], trend_text, 1, 1, 1, r, g, b)
                
                if change_percent then
                    GameTooltip:AddDoubleLine(L['Cambio:'], string.format('%.1f%%', change_percent * 100), 1, 1, 1, r, g, b)
                end
            end
        end
        ...
        if market_analysis.detect_market_manipulation then
            ...
            if is_manipulated then
                GameTooltip:AddLine(' ')
                GameTooltip:AddLine(L['⚠ Posible Manipulación'], 1, 0.65, 0)
                if reason then
        ...
        if market_analysis.get_average_daily_volume then
            ...
            if avg_volume and avg_volume > 0 then
                GameTooltip:AddLine(' ')
                GameTooltip:AddDoubleLine(L['Volumen Diario:'], string.format(L['%.1f items'], avg_volume), 1, 1, 1, 0.7, 0.7, 1)
            end
        end
    end
    ...
    if ml_patterns and ml_patterns.get_best_time_to_sell then
        ...
        if best_time and best_time.best_hour and best_time.confidence and best_time.confidence > 0.5 then
            GameTooltip:AddLine(' ')
            GameTooltip:AddLine(L['💡 Recomendación ML'], 0.7, 0.5, 1)
            ...
            GameTooltip:AddDoubleLine(L['Mejor momento venta:'], 
                string.format(L['%s %02d:00'], dia_nombre, best_time.best_hour),
                1, 1, 1, 0, 1, 0.5)
        end
    end
    ...
    if ml_patterns and ml_patterns.classify_item then
        ...
        if classification and classification ~= 'unknown' then
            GameTooltip:AddLine(' ')
            local class_text = classification == 'highly_profitable' and L['🏆 Muy Rentable'] or
                              classification == 'profitable' and L['✓ Rentable'] or
                              classification == 'marginally_profitable' and L['∼ Poco Rentable'] or
                              L['✖ No Rentable']
            ...
            if success_rate then
                GameTooltip:AddDoubleLine(L['Tasa de éxito:'], string.format('%.0f%%', success_rate * 100), 1, 1, 1, unpack(class_color))
            end
        end
    end
    ...
    if strategies and auction_info then
        GameTooltip:AddLine(' ')
        GameTooltip:AddLine(L['🎯 Estrategias'], 1, 0.82, 0)
        ...
        if strategies.evaluate_flipping_opportunity then
            ...
            if flip_result and flip_result.viable then
                GameTooltip:AddLine(L['✓ Flipping viable'], 0, 1, 0)
            end
        end
        ...
        if strategies.evaluate_sniping_opportunity then
            ...
            if snipe_result and snipe_result.viable then
                GameTooltip:AddLine(L['✓ Sniping viable'], 0, 1, 0)
            end
        end
    end
    ...
    if auction_info and auction_info.seller then
        GameTooltip:AddLine(' ')
        GameTooltip:AddDoubleLine(L['Vendedor:'], auction_info.seller, 1, 1, 1, 0.5, 0.5, 0.5)
    end
...
function M.mostrar_tooltip_simple(item_key, item_name)
    ...
    if market_value and market_value > 0 then
        ...
        GameTooltip:AddLine(L['Sin datos de mercado'], 0.8, 0.8, 0.8)
    end
...
aux.print(L['[TOOLTIPS_ADVANCED]|r Tooltips avanzados listos'])
